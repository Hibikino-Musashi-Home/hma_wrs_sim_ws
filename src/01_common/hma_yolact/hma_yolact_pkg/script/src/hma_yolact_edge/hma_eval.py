...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45):
    ...
    with timer.env('Copy'):
        if cfg.eval_mask_branch:
            ...
            ## add HMA Tomohiro ONO 20210108
            try:
                masks_numpy = [x.cpu().numpy() for x in t[3][:args.top_k]]
            except:
                masks_numpy = [x.cpu().detach().numpy() for x in t[3][:args.top_k]]
            ## end
    ...
    if num_dets_to_consider == 0:
        ...
        return (img_gpu * 255).byte().cpu().numpy(), classes, scores, boxes, masks_numpy
        ## end
    ...
    ## add HMA Tomohiro ONO 20210108
    # return img_numpy
    return img_numpy, classes, scores, boxes, masks_numpy
    ## end
